...
def _guidance_block(classification_guidance: str = "") -> str:
    ...
    return (
        ...
        "아래 원칙은 기존 원칙과 충돌하지 않는 범위에서 강하게 반영하라.\n"
        "--- 사용자 분류 원칙 ---\n"
        f"{text}\n"
        "--- 끝 ---"
    )
...
# agency / system nouns out of the filename via :mod:`folder1004.morph`
...
def build_rolling_prompt(
    ...
    *,
    ...
    return f"{ROLLING_SYSTEM}{hint}{_guidance_block(classification_guidance)}\n\n{instr}\n\n데이터:\n{body}"
...
def build_consolidation_prompt(
    categories: list[dict], classification_guidance: str = "",
) -> str:
    body = json.dumps({"categories": categories}, ensure_ascii=False, separators=(",", ":"))
    return f"{ROLLING_SYSTEM}{_guidance_block(classification_guidance)}\n\n{CONSOLIDATION_INSTRUCTION}\n\n데이터:\n{body}"
...
def build_rows(
    ...
    *,
    ...
    :func:`folder1004.cluster.signature`) — many real corpora have
