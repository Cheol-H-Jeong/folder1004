...
from .pipeline import apply_plan as apply_pipeline_plan
...
class ApplyPlanWorker(QtCore.QObject):
    stage_changed = QtCore.Signal(str, float)
    ...
    def run(self):
        try:
            def _progress(msg: str, pct: float):
                if self._cancel:
                    raise RuntimeError("canceled")
                self.stage_changed.emit(_stage_from_msg(msg), pct)
                self.status.emit(msg)

            op = apply_pipeline_plan(
                target_root=self.target,
                config=self.config,
                plan=self.plan,
                index_db=self.index_db,
            ...
            log_exception("ApplyPlanWorker.run failed", exc)
            text = str(exc) or type(exc).__name__
            if self._cancel or "cancel" in text.lower() or "취소" in text:
                self.failed.emit("정리를 취소했습니다")
            else:
