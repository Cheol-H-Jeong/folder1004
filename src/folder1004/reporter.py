...
def emit_markdown(op: OperationResult, out_dir: Path | None = None) -> Path:
    ...
    path = out_dir / f"Folder1004_Report_{stamp}.md"
...
def _build(op: OperationResult) -> str:
    lines: list[str] = []
    lines.append(f"# Folder1004 Report — {op.target_root}")
    lines.append("")
    ...
    lines.append(f"_Generated by Folder1004 at {datetime.now().astimezone().isoformat(timespec='seconds')}_")
    return "\n".join(lines)
