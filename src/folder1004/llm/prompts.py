...
- **잡파일은 단 하나의 "999. 기타" 카테고리에만 모은다 — 절대 두 개 이상 만들지 마라.**
...
- categories[].group 은 **관련성/주제별 묶음 번호**(1~999 정수, 0/누락 금지).
  같은 group 번호는 서로 비슷한 성격의 폴더끼리 부여한다. 예: 동일 고객사의 여러 사업이면
  같은 group, 내부 행사·잡파일류는 또 다른 group. 결과적으로 폴더 정렬을 도와준다.
  **모든 카테고리에 반드시 1~999 사이 숫자를 지정하라.** 잡파일/기타 카테고리는 999를 사용한다.
...
RECLASSIFY_HINT = (
    "사용자가 명시적으로 *모든 폴더 해체 후 재정리*를 선택했다. "
    "각 파일 path 의 부모 폴더 컴포넌트는 의도적으로 `[folder]` 자리표시자로 가려 두었다. "
    "단, previous_folder_hint 가 있으면 그것은 예전 폴더명에서 얻은 약한 참고 신호다. "
    "기존 폴더 구조를 그대로 유지하라는 뜻은 아니며, 파일명·본문 발췌·메타데이터와 충돌하면 무시하라. "
    "모든 파일을 파일 단위로 새 카테고리에 재배치하라."
)
...
- categories[].group(1~999), time_label, duration 도 부여하라.
